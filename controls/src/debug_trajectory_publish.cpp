...
private:
    ...
    std::vector<std::vector<std::tuple<float, float, float>>> trajectories = {// {
        ...
        {{0.0, 0.0, 0.9}, {0.3, 0.1, 0.7}, {0.6, 0.2, 0.5}, {0.9, 0.3, 0.3}, {1.2, 0.4, 0.2},
            {1.5, 0.5, 0.2}, {0.6, 0.5, -0.3},
            // {1.5, -0.5, 0.3},
            {1.5, -0.5, 0.3}, {0.0, 0.0, -.3}}};

    size_t current_trajectory = 0;

    void add_waypoints(Trajectory& trajectory,
        std::vector<std::tuple<float, float, float>> waypoints) {
        for (auto& waypoint: waypoints) {
            add_waypoint(trajectory, std::get<0>(waypoint), std::get<1>(waypoint),
                std::get<2>(waypoint));
        }
    }
